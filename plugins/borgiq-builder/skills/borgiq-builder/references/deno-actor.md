...
interface Request {
  ...
  /** secret values keyed by name — proxy placeholders for Server-side secrets (read-only) */
...
```typescript
const apiKey = req.credentials.OPENAI_API_KEY;    // Server-side: a placeholder, fine in an HTTPS header
const mongoUser = req.credentials.MONGO_USERNAME; // Sent to runtime: a database driver
const mongoPass = req.credentials.MONGO_PASSWORD; // needs the real value
```
